...
const Facts = [
  {
    fact: "Because of differences in gravity, a 200 pound person would only weigh 76 pounds on Mars.",
    image:
...
const Home = () => {
  return (
    <div className="h-screen w-full bg-science fixed">
      <Navbar />
      <h4 className="text-3xl font-serif p-2 font-semibold text-red-800 m-2 ">Fun Facts</h4>
      <motion.div  drag="x"   dragConstraints={{right:0,left:0}} className="overflow-hidden h-3/4 cursor-grab ml-25">
        <motion.div className="flex ">
          {Facts.map((fact) => {
            return <motion.div className=" p-5 w-full">
